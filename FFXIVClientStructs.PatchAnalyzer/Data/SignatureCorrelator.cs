...
public static class SignatureCorrelator {
    ...
    private static SignatureCatalogEntry Correlate(SignatureDefinition signature, DataCatalog catalog) {
        ...
        if ((kind == LocationKind.Instance || kind == LocationKind.VirtualTable) && catalog.DuplicateClassNames.Contains(nativeName))
            return new SignatureCatalogEntry(signature, DataCorrelationStatus.Ambiguous, null, $"Multiple class declarations match '{nativeName}'.");
