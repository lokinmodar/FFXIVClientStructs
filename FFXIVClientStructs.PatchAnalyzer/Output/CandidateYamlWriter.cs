...
public static class CandidateYamlWriter {
    ...
    public static string Render(PatchAnalysisResult result) {
        ...
        var replacements = BuildReplacements(result);
        var source = result.Data.SourceText;
    ...
    public static void Write(PatchAnalysisResult result, string inputPath, string outputPath, CancellationToken cancellationToken = default) {
        ...
        AtomicFileWriter.Write(outputPath, destination => Render(result, destination), cancellationToken);
    }

    private static IReadOnlyList<Replacement> BuildReplacements(PatchAnalysisResult result) {
        ...
        if (version is not null)
            replacements.Add(Replacement.FromSource(result.Data.SourceText, result.Data.VersionSourceSpan, version));
        ...
        foreach (var symbol in result.Symbols) {
            ...
            replacements.Add(Replacement.FromSource(result.Data.SourceText, location.SourceSpan, $"0x{currentAddress:X}"));
        }
